...
DEFAULT_ELEMENT_CONFIGS = {
    ...
    # Augmente la hauteur pour accueillir Distance, Allure, FC et Pente
    "Infos Texte": {
        "visible": True,
        "x": MARGIN,
        "y": INFO_TEXT_Y,
        "width": 368,
        "height": INFO_TEXT_HEIGHT,  # 7 lignes : Vitesse, Distance, Altitude, Heure, Pente, Allure, FC
    },
}
...
def prepare_track_arrays(
    ...
    if len(interp_eles) > 1:
        segment_distances = haversine_np(
            interp_lats[:-1], interp_lons[:-1], interp_lats[1:], interp_lons[1:]
        )
        interp_distances = np.concatenate((
            [0.0],
            np.cumsum(segment_distances, dtype=float),
        ))
        elev_diff = np.diff(interp_eles)
        seg_slopes = np.where(segment_distances > 0, elev_diff / segment_distances, 0.0) * 100.0
        interp_slopes = np.insert(seg_slopes, 0, seg_slopes[0] if seg_slopes.size else 0.0)
    else:
        interp_distances = np.zeros_like(interp_eles)
...
def draw_info_text(draw, speed, distance_m, altitude, slope, current_time, draw_area, font, tz, text_color):
    ...
    draw.text((x, y0), f"Vitesse : {speed:.0f} km/h", font=font, fill=text_color)
    draw.text((x, y0 + line_spacing), f"Distance : {distance_km:.2f} km", font=font, fill=text_color)
    draw.text((x, y0 + 2 * line_spacing), f"Altitude : {altitude:.0f} m", font=font, fill=text_color)
    draw.text((x, y0 + 3 * line_spacing), f"Heure : {display_time}", font=font, fill=text_color)
    draw.text((x, y0 + 4 * line_spacing), f"Pente : {slope:.1f} %", font=font, fill=text_color)
    ...
    try:
        ...
        for frame_count, global_idx in enumerate(
            ...
            if info_area.get("visible", False):
                draw_info_text(draw,
                               float(interp_speeds[global_idx]),
                               float(interp_distances[global_idx]),
...
def render_first_frame_image(
    ...
    if info_area.get("visible", False):
        draw_info_text(draw,
                       float(interp_speeds[current_idx]),
                       float(interp_distances[current_idx]),
